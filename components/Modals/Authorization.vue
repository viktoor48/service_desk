...
const authorizationValidationSchema = yup.object().shape({
  ...
  userPassword: yup.string().required().min(10, 'Пароль должен содержать 10 цифр!'),
})

const { values, meta } = useForm<WriteUsForm>({ validationSchema: authorizationValidationSchema })
...
function sendForm() {
  ...
  fetch(`${apiURL}/user`, {
    ...
    .catch(() => {
      requestErrorMessage.value = 'Не удалось авторизоваться, попробуйте другие данные.'
    })
    .finally(() => {
